...
export default function CustomModelNode({ data }) {
  const disabled = data?.enabled === false
  const ui = useWorkflowUI()
  const modelLabel = data?.model || data?.model_name || ui?.t?.('custom_model_subtitle') || 'custom plugin'
  useRenderTimer(`Node:CustomModel`)

  return (
    <div
      className={`rounded-2xl border border-slate-700 bg-slate-900/90 px-4 py-3 shadow-lg shadow-black/30 ${disabled ? 'opacity-50' : ''}`}
    >
      <div className="text-sm font-semibold text-slate-100">{ui?.t?.('node_custom_model') || 'Custom Model'}</div>
